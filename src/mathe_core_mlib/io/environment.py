...
def snapshot_environment(save_path: Path) -> Path:
    """
    ...
    try:
        ...
        uv_output = subprocess.check_output(["uv", "pip", "list", "--format=json"], stderr=subprocess.DEVNULL).decode("utf-8")
    ...
    output_file = save_path / "env_snapshot.json"
...
def enforce_clean_environment(snapshot_path: Path, strict: bool = True):
    """
    ...
    """
    snapshot_file = snapshot_path / "env_snapshot.json"
    ...
    if sim_git.get("is_dirty", False):
        print(f"\n{Fore.YELLOW}{'=' * 60}")
        print("⚠️  WARNING: O código do simulador possui alterações não commitadas.")
    ...
    if dirty_libs:
        print(f"\n{Fore.RED}{'=' * 60}")
        print("🛑 CRITICAL: AMBIENTE 'SUJO' DETECTADO")
        print(f"{'=' * 60}{Style.RESET_ALL}")
